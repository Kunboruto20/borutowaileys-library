 ...
 * @param {string} authDir - Directorul cu fișierele de autentificare
 * @returns {Promise<boolean>} - True dacă curățarea a fost realizată cu succes
 */
const clearCorruptedAuthSession = async (authDir) => {
    try {
        if (!fs.existsSync(authDir)) {
            return true; // Nu există director, deci e deja "curat"
        }
        ...
        for (const file of files) {
            // Șterge fișierele de credențiale corupte
        ...
        // Recreează directorul dacă a fost șters complet
        if (!fs.existsSync(authDir)) {
