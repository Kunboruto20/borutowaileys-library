...
const { clearCorruptedAuthSession } = require('./Utils/auth-utils');
const { WhatsAppErrorHandler, isAuthClearingError, getErrorFixAction } = require('./Utils/whatsapp-error-handler');
...
/**
 * Creează un socket WhatsApp cu funcționalități îmbunătățite și fix-uri pentru erori
 * @param {Object} options Opțiunile pentru socket
 * @returns {Object} Socket-ul îmbunătățit
 */
function makeEnhancedWASocket(options = {}) {
  // Inițializăm error handler-ul pentru fix-urile WhatsApp
  const errorHandler = new WhatsAppErrorHandler(options.logger);
  
  // Aplicăm configurația optimizată pentru fix-urile de erori
  const optimizedOptions = errorHandler.getOptimizedSocketConfig(options);
  
  // Creăm socket-ul de bază cu configurația optimizată
  const sock = makeWASocket(optimizedOptions);
  ...
  sock.errorHandler = errorHandler;
  
  // Adăugăm gestionarea avansată a erorilor de conexiune
  sock.ev.on('connection.update', (update) => {
    const { connection, lastDisconnect } = update;
    
    if (connection === 'close' && lastDisconnect?.error) {
      const errorInfo = errorHandler.processConnectionError(lastDisconnect.error);
      
      // Emit eveniment personalizat cu informații despre fix
      sock.ev.emit('connection.error_processed', {
        originalError: lastDisconnect.error,
        errorInfo,
        suggestedAction: errorInfo.fixAction,
        shouldClearAuth: errorInfo.shouldClearAuth
      });
      
      // Auto-clear auth dacă este necesar (pentru HTTP 428, 405, etc.)
      if (errorInfo.shouldClearAuth && options.autoClearAuth !== false) {
        sock.ev.emit('auth.clear_required', errorInfo);
      }
    } else if (connection === 'open') {
      // Resetăm contorul de încercări la conexiune reușită
  ...
  // Metode pentru gestionarea erorilor WhatsApp
  sock.isAuthClearingError = (code, reason) => {
    return isAuthClearingError(code, reason);
  };
  
  sock.getErrorFixAction = (code, reason) => {
    return getErrorFixAction(code, reason);
  };
  
  sock.processConnectionError = (error) => {
    return errorHandler.processConnectionError(error);
  };
  
  sock.hasReachedMaxReconnectAttempts = (maxAttempts) => {
    return errorHandler.hasReachedMaxAttempts('default', maxAttempts);
  };
...
module.exports = {
  ...
  // Fix-uri pentru erori WhatsApp (HTTP 405, 428, 500, WebSocket 1006)
  WhatsAppErrorHandler,
  isAuthClearingError,
  getErrorFixAction,
  clearCorruptedAuthSession
};