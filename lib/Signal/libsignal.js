...
function makeLibSignalRepository(auth) {
    ...
    return {
        ...
        async injectE2ESession({ jid, session }) {
            const builder = new libsignal.SessionBuilder(storage, jidToSignalProtocolAddress(jid));
...
/**
 * Convertește JID-ul într-o adresă pentru libsignal.
 */
...
/**
 * Convertește un JID de utilizator într-un nume unic pentru sender-key (folosit la chat-urile de grup).
 */
...
/**
 * Implementarea unei stocări pentru libsignal, cu câteva îmbunătățiri:
 * - Caching pentru sesiunile încărcate (pentru a evita apelurile repetate)
 * - Retry adaptiv pentru încărcarea sesiunilor (cu backoff exponențial)
 */
function signalStorage({ creds, keys }) {
    // Cache intern pentru sesiunile deja încărcate
    const sessionCache = new Map();
    return {
        loadSession: async (id) => {
            if (sessionCache.has(id)) {
                // Sesiunea a fost deja încărcată din cache
                return sessionCache.get(id);
            }
            // Retry adaptiv: încercăm de 3 ori cu delay crescător înainte de a renunța
            let retries = 3;
            let delayMs = 100;
            let sess;
            while (retries > 0) {
